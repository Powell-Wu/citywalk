...
test('all narrative illustrations and gesture code are available without network',async()=>{const w=await worker();let install;w.handlers.install({waitUntil:p=>install=p});await install;const count=w.network();for(const name of ['story-scene.webp','story-talk.webp','story-event.webp','story.css','swipe.mjs']){let response;w.handlers.fetch({request:{url:'https://walk.example/'+name,method:'GET',mode:'cors'},respondWith:p=>response=p});assert.equal((await response).status,200);}assert.equal(w.network(),count);});
