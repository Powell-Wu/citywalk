...
case'replace':await mutate(s=>replaceCard(s,d.session,d.slot,d.card));toast('换一张，按你们的心情来。');break;
case'complete':{const input=document.querySelector('#actual-cost');if(input&&!input.reportValidity())return;await mutate(s=>complete(s,d.session,d.slot,d.card,input?Number(input.value):0));ui.focusSlot=null;toast('这件小事，记下了。');break;}
...
document.addEventListener('submit',async event=>{const f=event.target;if(!['setup-form','finish-form','reward-form','card-form','filters-form'].includes(f.id))return;event.preventDefault();if(busy||!f.reportValidity())return;const data=new FormData(f),submit=f.querySelector('[type="submit"]');submit.disabled=true;try{switch(f.id){
